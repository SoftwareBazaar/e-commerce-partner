...
const CustomEA = () => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const strategy = String(fd.get("strategy"));
    const budgetRange = String(fd.get("budget"));
    ...
    const requestId = data?.[0]?.id || `REQ-${Date.now()}`;
    const emailResult = await sendCustomEARequestConfirmation(
      clientEmail,
      clientName,
      requestId,
      strategy,
    ...
    if (!emailResult.success) {
      ...
      toast({ 
        ...
        description: "We'll review and get back within 24 hours. (Email notification pending)",
        variant: "default"
      });
    } else {
      toast({ 
        title: "Request received", 
        description: "Confirmation email sent. We'll review and get back within 24 hours." 
      });
    }
