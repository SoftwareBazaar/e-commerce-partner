-- ============================================================================
-- ROBERT TRADING TOOLS - COMPLETE DATABASE SCHEMA (SAFE VERSION)
-- ============================================================================
...
CREATE POLICY IF NOT EXISTS "Allow public read access to mentorship_packages" ON mentorship_packages
  FOR SELECT USING (is_active = true);

-- Blog posts - Public read published posts
CREATE POLICY IF NOT EXISTS "Allow public read access to published blog posts" ON blog_posts
  FOR SELECT USING (status = 'published');

-- Product reviews - Public read approved reviews
CREATE POLICY IF NOT EXISTS "Allow public read access to approved reviews" ON product_reviews
  FOR SELECT USING (status = 'approved');

-- Email templates - Public read
CREATE POLICY IF NOT EXISTS "Allow public read access to email_templates" ON email_templates
...
CREATE POLICY IF NOT EXISTS "Users can view own custom EA requests" ON custom_ea_requests
  FOR SELECT USING (auth.uid()::text = user_id::text OR auth.role() = 'authenticated');
...
CREATE POLICY IF NOT EXISTS "Users can view own cart" ON cart_items
  FOR SELECT USING (auth.uid()::text = user_id::text);

-- Wishlist - Users can view own wishlist
CREATE POLICY IF NOT EXISTS "Users can view own wishlist" ON wishlist
  FOR SELECT USING (auth.uid()::text = user_id::text);
